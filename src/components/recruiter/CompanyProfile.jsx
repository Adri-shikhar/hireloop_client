...
export default function CompanyProfile() {
  ...
  const handleSubmit = async (e) => {
    ...
    const updated = await getCompany(user.user?.id);
  ...
  if (loading || user.isLoading) {
    return (
      <div className="rd-company-shell">
        <PageHeader variant="recruiter" title="My Company" description="Loading your company profile..." />
      </div>
    );
  }
  ...
  if (!company?._id && !isEditing) {
    return (
      <div className="rd-company-shell">
        <PageHeader variant="recruiter" title="My Company" description="Set up and manage your business profile to start hiring on HireLoop." />
        <DashboardEmptyState
          icon={Building2}

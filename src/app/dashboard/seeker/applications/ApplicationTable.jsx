...
function getStatusChip(status = "Applied") {
    ...
    return (
      <>
        <RecruiterPageHeader
          ...
          description="Track every role you have applied to on HireLoop."
        />
        <DashboardEmptyState
          icon={FileText}
          title="No applications yet"
          description="Browse open roles and apply to start tracking your job search here."
          action={
  ...
  return (
    <>
      ...
      <div className="rd-jobs-panel">
        ...
        <div className="jobs-table-wrap" style={{ padding: "0 4px 4px" }}>
          <Table
            className="w-full"
            classNames={{
              base: "bg-transparent",
              ...
              <Table.Content aria-label="Job applications tracking table">
                ...
                <Table.Body emptyContent="No applications found.">
                  {applications.map((application) => {
                    const style = getJobStyle(application.jobTitle);
                    const workType = application.workType || application.jobType || "Full-time";
                    const locationType = application.locationType || (application.isRemote ? "Remote" : "On-site");
                    const status = application.status || "Applied";
                    const appliedAt = application.createdAt?.$date || application.createdAt;
                    const jobId = application.jobId || application._id;

                    return (
                      <Table.Row key={String(application._id)}>
                        <Table.Cell>
                        ...
                        <Table.Cell className="text-right">
                          {jobId ? (
                            ...
                            <Button size="sm" variant="light" isDisabled className="text-zinc-500 text-sm">
                              Details
                            </Button>
                          )}
                        </Table.Cell>
